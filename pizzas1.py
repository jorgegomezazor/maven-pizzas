...
def analisis_nulls(order_details, orders, pizza_types): #analizo los nulls
    analisis = [order_details, orders, pizza_types] #los guardo en una lista
    df_num = 0 #contador de dataframes
    analisis_str = ['order_details', 'orders', 'pizza_types'] #los guardo en una lista de strings
    numero_nulls = {} 
    for df in analisis:
        cols = df.columns 
        nuls = df[cols].isnull() #guardo los nulls en una variable
        missing = 0
        for nul in nuls: #recorro los nulls
            for i in range(len(nul)): 
                if nul[i] == True:
                    missing += 1 #cuento los nulls
        numero_nulls[analisis_str[df_num]] = missing #guardo los nulls en un diccionario
        df_num += 1
    print(numero_nulls) #imprimo el diccionario
    return
